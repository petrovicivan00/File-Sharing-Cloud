...
public class MultipleServentStarter {
	...
	private static void startServentTest(String testName) {
		...
		ProcessBuilder bsBuilder = new ProcessBuilder("java", "-cp", "out/production/kids_proj_ivan_petrovic_rn6319", "app.BootstrapServer", String.valueOf(AppConfig.BOOTSTRAP_PORT));
		try {
		...
		for(int i = 0; i < serventCount; i++) {
			try {
				ProcessBuilder builder = new ProcessBuilder("java", "-cp", "out/production/kids_proj_ivan_petrovic_rn6319", "app.ServentSingle",
				...
				AppConfig.timestampedStandardPrint("Error while starting servents");
				e.printStackTrace();
			}
